...
def write_to_excel():
    ...
    row0 = ['期号', '中奖号码', '总和', '总销售额(元)', '直选注数', '直选奖金', '组选3注数', '组选3奖金', '组选6注数',
            '组选6奖金', '开奖日期']  # 把所有列名做成一个list表
    # 写入第一行
    ...
    url = 'http://datachart.500.com/pls/history/inc/history.php?limit=15116&start=24136&end=24164'
    ...
    try:
        f.save('validation.xls')
        print('写入EXCEL表validation.xls成功!')
    except:
        print('写入EXCEL表失败')
